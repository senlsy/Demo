...
public class PinnedSectionListView extends ListView {
    ...
    private GradientDrawable mShadowDrawable;//阴影
    private int mSectionsDistanceY;//下一个SECTION item的top与悬浮view bottom的距离
    ...
    private final OnScrollListener mOnScrollListener = new OnScrollListener() {
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            Log.d("lintest", String.format("f=%s,vc=%s,tc=%s,isSection=%s", firstVisibleItem, visibleItemCount, totalItemCount, isFirstVisibleItemSection));

            if (isFirstVisibleItemSection) {
                View sectionView = getChildAt(0);
                if (sectionView.getTop() == getPaddingTop()) { // view sticks to the top, no need for pinned shadow
                    destroyPinnedShadow();//当section类型的item和listview的top重合重合时就不需要悬浮view了
     ...
     * 创建当前section对应的悬浮view，并且结算与下一section的距离。
     * 当前悬浮view被layout在listview的顶部，当下一section的top与悬浮veiw底部的距离为负数时，说明悬浮view需要进行偏移mTranslateY
     * 在dispatchDraw()回调时进行绘制偏移，layout的位置是没有变化的
     */
    void ensureShadowForPosition(int currentSectionPosition, int firstVisibleItem, int visibleItemCount) {
        if (visibleItemCount < 2) { // no need for creating shadow at all, we have a single visible item
            destroyPinnedShadow();
            return;
        }

        if (mPinnedSection != null && mPinnedSection.position != currentSectionPosition) { // invalidate shadow, if required
            destroyPinnedShadow();
        }
        ...
        int nextPosition = currentSectionPosition + 1;

        if (nextPosition < getCount()) {

            int nextSectionPosition = findNextSectionPosition(nextPosition,
    ...
    /*获取下一条SCTION的位置*/
    int findNextSectionPosition(int currentPos, int lastVisiblePos) {
        ListAdapter adapter = getAdapter();
        ...
        if (currentPos + lastVisiblePos >= adapterDataCount) {//added to prevent index Outofbound (in case)
            lastVisiblePos = adapterDataCount - currentPos;
        }

        for (int childIndex = 0; childIndex < lastVisiblePos; childIndex++) {
            int position = currentPos + childIndex;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mPinnedSection != null) {
            ...
            // draw child
            canvas.save();
            int clipHeight = view.getHeight() + (mShadowDrawable == null ? 0 : Math.min(mShadowHeight, mSectionsDistanceY));//view高度+阴影高度

            //todo 裁剪的虽然是从pLeft和pTop开始，还是得挪动坐标系
            canvas.clipRect(pLeft, pTop, pLeft + view.getWidth(), pTop + clipHeight);
            canvas.translate(pLeft, pTop + mTranslateY);//需要加上偏移量
            ...
            if (mShadowDrawable != null && mSectionsDistanceY > 0) {
                ...
                mShadowDrawable.draw(canvas);//todo 没移动坐标系，直接画上去
            }
    ...
    private final PointF mTouchPoint = new PointF();//action down的坐标
    private int mTouchSlop;//判定是否是滑动的最小移动量
    private View mTouchTarget;//当前悬浮的view
    private MotionEvent mDownEvent;//down事件
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        //down之后mTouchTarget不为null，down事件就被拦截了
        if (mTouchTarget != null) {

            if (isPinnedViewTouched(mTouchTarget, x, y)) { // forward event to pinned view
                mTouchTarget.dispatchTouchEvent(ev);//讲事件分发到悬浮view上
            }

            //todo 下面是决定是否将事件还给listviwe
            if (action == MotionEvent.ACTION_MOVE) {
                ...
                if (Math.abs(y - mTouchPoint.y) > mTouchSlop) {
                    ...
                    event.setAction(MotionEvent.ACTION_CANCEL);//todo 发送cancel事件给悬浮view
                    mTouchTarget.dispatchTouchEvent(event);
                    event.recycle();

                    // provide correct sequence to super class for further handling
                    super.dispatchTouchEvent(mDownEvent);//todo 补发down事件给listview，并将mTouchTarget置空，后续的事件就有super自个处理
                    super.dispatchTouchEvent(ev);
                    clearTouchTarget();
                }
                //todo 横向滑动的事件没有处理

            } else if (action == MotionEvent.ACTION_UP) { // perform onClick on pinned view

                super.dispatchTouchEvent(ev);//下发到listview的子view
    ...
    //x,y坐标是否落在该view上，这里加上了view偏移量的处理，view的layout位置是不变的，但是绘制的时候发生偏移，所以要矫正区域
