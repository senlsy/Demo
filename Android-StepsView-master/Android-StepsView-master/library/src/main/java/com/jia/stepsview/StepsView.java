...
public class StepsView extends LinearLayout implements StepsViewIndicator.OnDrawListener
{
    ...
    private int mBarColorIndicator = Color.BLACK;// 未完成节点颜色
    private int mProgressColorIndicator = Color.YELLOW;// 已完成节点颜色
    private int mLabelColorIndicator = Color.BLACK;// 未完成label颜色
    private int mLabelColorComplete = Color.YELLOW;// 已完成label颜色
    private int mCompletedPosition = 0;// 完成位置
    ...
    //
    public int getLabelColorComplete() {
        return mLabelColorComplete;
    }
    
    public StepsView setLabelColorComplete(int mLabelColorComplete) {
        this.mLabelColorComplete = mLabelColorComplete;
        return this;
    }
    ...
    public void drawView() {
        if(mLabels == null || mLabels.length < 2){
        ...
        mStepsViewIndicator.requestLayout();
        ...
        if(mLabels != null){
            ...
            for(int i = 0;i < mLabels.length;i++){
                ...
                if(i <= mCompletedPosition){
                    ...
                    textView.setTextColor(mLabelColorComplete);
                }
                else{
